...
# 按Variable分组拼接
combined_plots <- analysis_data %>%
  group_by(Variable) %>%
  group_map(~ {
    # 水平拼接同Variable的图
    row_plot <- wrap_plots(.x$plot, nrow = 1)
    return(row_plot)
  }) %>%
  # 垂直拼接不同Variable的图形
...
combined_plots <- analysis_data %>% 
  group_by(Variable) %>% 
  summarise(
    row_plot = wrap_plots(plot, nrow = 1) +  # 更安全的方式
      plot_annotation(tag_levels = "A")      # 添加子图标签
  ) %>% 
  pull(row_plot) %>% 
  wrap_plots(ncol = 1) +                    # 垂直排列所有行
