...
variables <- c("AIP", "TyG", "eGFR", "eGDR_BMI")
stage_pairs <- list(
  c(0, 1), c(1, 2), c(2, 3), 
  c(3, "4a"), c("4a", "4b")
)
...
for (data_source in data_sources) {
  ...
  for (var in variables) {
    for (i in seq_along(stage_pairs)) {
      ...
      # 有效性检查
      if (nrow(subset_data) < 20 || nlevels(subset_data$CKM_stage_ab) != 2) {
      ...
      # 在模型拟合前添加异常处理
      final_fit <- tryCatch(
        lrm(final_formula, data = subset_data, x = TRUE, y = TRUE),
        error = function(e) {
          message(paste("模型拟合失败:", data_source, var, stage_label, "原因:", e$message))
          return(NULL)
        }
      )
      ...
      p <- ggplot(pred, aes(x = .data[[var]], y = yhat)) +  # 使用.data代词
        ...
        labs(caption = sprintf("Overall P: %s | Nonlinear P: %s", 
