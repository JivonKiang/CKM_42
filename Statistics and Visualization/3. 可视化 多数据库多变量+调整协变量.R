...
# 在原有代码基础上增加以下调整部分
adjust_vars <- c("Status","Age","Sex","Marital_status","Education_level","Smoking",
...
# 主循环
# 修改主循环结构（在原有循环外添加协变量循环）
for (adj_var in c("unadjusted", adjust_vars)) { # 增加未调整模型
  # 创建调整模型专属目录
  if (adj_var != "unadjusted") {
    model_dir <- file.path("RCS", adj_var)
  ...
  for (var in variable) {
    for (i in seq_along(stage_pairs)) {
      ...
      # 修改模型公式
      if (adj_var == "unadjusted") {
        formula_str <- paste("CKM_stage_ab ~ rcs(", var, ",", best_k, ")")
      } else {
        formula_str <- paste("CKM_stage_ab ~ rcs(", var, ",", best_k, ") +", adj_var)
      }
      
      # 模型拟合（增加协变量处理）
      final_fit <- tryCatch(
        lrm(as.formula(formula_str), 
            data = subset_data, 
            x=TRUE, y=TRUE, maxit=200),
        error = function(e) NULL
      )
      ...
      # 保存图形
      # 修改图形保存路径
      filename <- file.path(model_dir, 
                            paste0(data_source, "_", var, "_", stage_label, ".png"))
      ggsave(filename, plot = p, width = 6, height = 4, dpi = 300)
      
      # 保存结果到Excel
      openxlsx::write.xlsx(
        stat_results,
        file.path(model_dir, "RCS_Statistical_Results.xlsx"),  # 分模型保存
        colNames = TRUE,
        rowNames = FALSE,
        keepNA = TRUE
      )
...
###最后一个变量是Hypertension，所以所有结果都在Hypertension中
#比较调整前后OR值变化：>10%说明有混杂效应
#关注调整后P值变化方向：
