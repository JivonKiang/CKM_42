...
data1 <- data1[, -which(names(data1) == "Race")]
...
colnames(data1)
categorical_vars <- c("Sex", "Marital_status", "Education_level", "Hukou_status", 
                      "Smoking", "Drinking","Sleep_health",
                      "Diabetes", "Cancer", "Stroke", "Hypertension",
                      "Heart_disease",
                      "Dyslipidemia", 
                      "Kidney_disease",
                      "Chest_pain",
                      "Chest_Pains_When_Climbing",
...
# 移除含NA的变量（根据summary结果筛选）或者提前去除多重共线性的变量
...
data1 <- data1[, !(names(data1) %in% exclude_vars)]
