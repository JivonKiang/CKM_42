...
library(openxlsx)
rm(list=ls())
#setwd("~/20250110 Cardiovascular-Kidney-Metabolic Syndrome, CKM/NHANES")
# 读取数据
...
# 修改函数以支持并行化
generate_operations_parallel <- function(data) {
  all_operations <- list()
  ...
  # 设置并行后端
  cl <- makeCluster(12)  # 使用除了主核之外的所有核心
  doParallel::registerDoParallel(cl)
  
  # 并行化生成运算组合
  foreach(var1_index = 1:num_cols, .combine = c) %:%  # 外层循环并行化
    foreach(var2_index = 1:num_cols, .combine = c) %:%
    foreach(op1 = operators, .combine = c) %:%
    foreach(var3_index = 1:num_cols, .combine = c) %:%
    foreach(number_choose = 1:2, .combine = c) %:%
    foreach(op2 = operators, .combine = c) %:%
    foreach(ln_op = ln_or_identity, .combine = c) %dopar% {
      operation_str <- ""
      
      # 第一部分：var_1~var_n中的任意一个变量相乘或相除var_1~var_n中的任意一个变量
      ...
      # 第三部分：将前一步的运算结果相乘或相除var_1~var_n中的任意一个变量
...
# 假设已经有了名为data的数据框
all_operations <- generate_operations_parallel(data)
