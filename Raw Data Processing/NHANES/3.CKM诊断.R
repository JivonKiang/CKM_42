...
data <- data %>% 
  mutate(Subclinical_evidence = case_when(
    #`Chest pain` == "1_Yes" | `Severe chest pain` == "1_Yes" | 
      `Breath shortness` == "1_Yes" ~ "Positive",
    TRUE ~ "Negative"
  ))
...
data <- data %>% 
  mutate(Clinical_evidence = case_when(
    `Stroke` == "1_Yes" | `Congestiveheart failure` == "1_Yes" | `Myocardial infarction` == "1_Yes"  | `Angina pectoris` == "1_Yes"~ "Positive",
    TRUE ~ "Negative"
  ))
